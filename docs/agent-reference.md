...
Expanded tool-routing and operational reference moved out of `configs/AGENTS.md` to keep the always-loaded agent contract small. Load this file only when you need exact one-liners or detailed routing notes.
...
| Plan/PR understanding before merge/handoff | `$grill-me` | Matt Pocock upstream standalone skill installed by `scripts/skills.sh`; one-question-at-a-time interrogation with recommended answers. |
...
- **graphify** (Claude/Codex skill, `/graphify`) — build a queryable knowledge
...
- **headroom** — default model-traffic compression for long Claude/Codex/OMX
  sessions when installed. `.zshrc` routes `claude`, `codex`, and `omx`
  through Headroom wrappers automatically; explicit `claudeh` / `codexh` /
  `omxh` also work. Bypass with `HEADROOM_DEFAULT=0`, per-tool
  `HEADROOM_CLAUDE=0` / `HEADROOM_CODEX=0` / `HEADROOM_OMX=0`, or
  `command claude|codex|omx`. `codexh` and `omxh` temporarily prepare Codex's
  Headroom provider and share a lock/refcount under
  `~/.local/state/headroom-agent/codex-config`, so concurrent cmux tabs do not
  unwrap each other's `~/.codex/config.toml`. Keep the tracked Codex template's
  top-level `model_provider` unset; Headroom injects a temporary provider and a
  template-level provider creates duplicate TOML keys. Default mode is
  `HEADROOM_MODE=cache` to preserve provider prompt-cache economics; set
  `HEADROOM_MODE=token` only when maximum compression matters more than
  prefix-cache stability. Owl is optional: use it as an extra Claude cache
  signal when present, but do not treat missing `owl-rs` as a blocker.
- **agent status labels** — `scripts/statusline.sh` installs
