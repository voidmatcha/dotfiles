...
- 기술문서 한국어(윤문·번역 후수정·delta·코퍼스·termbase·eval)는 이 스킬에 없음 → `korean-tech-humanizer` 스킬 참조


## 기술문서 한국어 → `korean-tech-humanizer`로 라우팅

기술문서 한국어 윤문, 영어 원문+한국어 초벌의 번역 후수정, 좋은 번역과 LLM 초벌의 delta audit은 이 스킬에서 처리하지 않고 **`korean-tech-humanizer`** 스킬로 보낸다. 코드/API/URL/숫자/버전/제품명 보존, 용어 삼분법, 기술한국어 코퍼스·termbase·eval이 거기에 있다. (2026-07 분리 — 경위는 korean-tech-humanizer의 `references/repo-split-plan.md`.)
