...
## 6. Technical Korean / 번역 후수정 레시피 (참고 사본 — `korean-tech-humanizer`로 이전됨)

> **2026-07 이후 이 레시피의 권위 사본은 `korean-tech-humanizer` 스킬이 소유한다.** 기술문서 윤문·번역 후수정·delta audit 입력은 그 스킬로 라우팅한다. 아래는 humanize-korean에 남겨둔 참고용 사본이며, 최신 규칙은 korean-tech-humanizer의 `references/technical-korean-style.md`·`tech-translation-delta.md`를 따른다.

기술문서에서는 자연스러움보다 **의미 보존과 식별자 보존**을 먼저 잠근다. 영어 원문이 같이 있으면 한국어 초벌만 보지 말고 원문 조건·부정·수치·버전을 기준으로 검수한다.

### 6.1 식별자 보존

- 코드 블록/인라인 코드/URL/파일 경로/CLI flag/env var/API 이름은 그대로 둔다.
- 제품명·패키지명·프레임워크명은 독자가 검색하는 형태를 우선한다.
- 한국어 조사를 붙일 때만 형태를 조정한다. 예: `kubectl`을 실행합니다, `Promise`를 반환합니다.

### 6.2 가능/제공/활용 표현 줄이기

나쁜 초벌:

> 이 API는 사용자가 요청을 수행할 수 있도록 하는 기능을 제공합니다.

수정 방향:

> 이 API로 요청을 보낼 수 있습니다.

규칙:

- `you can`은 “할 수 있습니다”로 옮기기 전에 절차문인지 설명문인지 본다.
- `provides/allows/enables`는 “제공/가능하게 함”보다 실제 동작 동사를 찾는다.
- “활용하다/수행하다/진행하다”가 반복되면 “쓰다/보내다/읽다/설정하다/실행하다/반환하다”로 구체화한다.

### 6.3 영어 긴 문장 처리

- 한 영어 문장에 조건·목적·결과가 모두 있으면 한국어에서는 2문장으로 나눠도 된다.
- 관계절을 그대로 앞에 길게 붙이지 않는다.
- 독자가 먼저 알아야 하는 대상 → 조건 → 동작 → 결과 순서로 재배열한다.

### 6.4 경고문/제약 보존

- `must`, `should`, `may`, `cannot`의 강도를 바꾸지 않는다.
- 보안/데이터 손실/호환성 경고는 부드럽게 만들지 않는다.
- “주의하세요”만 쓰지 말고 어떤 조건에서 어떤 문제가 생기는지 명시한다.

### 6.5 Delta audit 산출

좋은 한국어 번역과 LLM 초벌 번역을 비교하는 작업이면 결과를 문장별 수정본으로만 끝내지 말고, 반복 가능한 규칙 후보를 남긴다.

```markdown
Observed delta: D5 가능 표현 절제
Baseline: “사용할 수 있도록 합니다” 반복
Reference-like rewrite: “사용합니다/설정합니다/보냅니다”로 동작 동사화
Candidate rule: 절차문에서는 가능 표현보다 직접 동작을 우선한다.
```

### 6.6 Pilot 002 승격 규칙

- **Excerpt QA**: 번역 후수정/eval 후보에서 한국어 참조가 영어 그대로, 메타데이터뿐, 목차뿐, 다른 섹션이면 그 pair는 사용하지 않는다.
- **구조 재작성 허용**: 원문 사실·조건·강도·식별자를 보존하는 한, 한국어에서는 긴 영어 절을 나누고 어순을 바꾼다.
- **모달 압축**: `can/allow/enable/provide`를 자동으로 “할 수 있습니다/가능하게 합니다/제공합니다”로 옮기지 않는다. 기술 동작이 보이면 “설정합니다/적용합니다/지원합니다/보냅니다/반환합니다/사용합니다”처럼 구체 동사로 쓴다.
- **절차문 톤**: UI/API 단계는 “당신은 …할 수 있습니다”보다 “대상을 선택하고 …을 적용합니다”처럼 대상·동작·결과 중심으로 쓴다.
