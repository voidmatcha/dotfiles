...
## T. Technical Korean guard (v1.6 draft)

Fast path 입력이 README, API/CLI 문서, 기술 블로그, 프롬프트/스킬 문서, 영어 원문+한국어 초벌 번역이면 일반 AI 티 제거 전에 다음을 잠근다.

- 코드 블록, 인라인 코드, URL, 파일 경로, CLI 명령, 환경 변수, API/패키지/제품명은 바꾸지 않는다.
- 숫자, 버전, 조건, 부정, 보안/주의 문구의 강도를 보존한다.
- `you can`, `allows`, `enables`, `provides`를 기계적으로 “할 수 있습니다/가능하게 합니다/제공합니다”로 반복하지 않는다.
- “수행/활용/진행/제공” 명사구가 반복되면 실제 동작 동사로 바꾼다.
- 용어는 canonical English / established Korean / awkward calque로 나누고 문서 안 일관성을 우선한다.

긴 기술문서, 원문 대조, 또는 좋은 번역과 LLM 초벌의 차이 분석은 Fast path에서 끝내지 말고 `korean-tech-humanizer` 스킬로 넘긴다 (기술한국어 스타일·번역 delta·termbase가 거기에 있다).

### T-1. Pilot 002 promoted rules

- 기술문서 excerpt/eval을 만들 때 KO가 영어 그대로이거나 메타데이터/목차뿐이면 그 pair는 버린다.
- 영어 절 순서를 그대로 보존하려 하지 말고, 사실·조건·식별자·강도만 보존되면 한국어 문장으로 분할/재배열한다.
- `can/allow/enable/provide`는 “할 수 있습니다/가능하게 합니다/제공합니다”로 자동 대응하지 말고 실제 동작 동사로 줄인다.
- UI/API 절차문은 “you”를 빼고 대상 → 동작 → 결과 순서로 쓴다.
