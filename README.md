...
- Skills — agent-skills, ai-slop-cleaner, clarify, code-review, doc-coauthoring, e2e-skills, frontend-design, humanizer, im-not-ai, internal-comms, karpathy-guidelines, mcp-builder, obsidian-skills, project-session-manager (`/oh-my-claudecode:psm`), security-best-practices, skill-creator, ui-clone-skills, webapp-testing, plus repo-local skills (`dotfiles-verify`, `agent-usage-audit`, `code-intel-doctor`, `work-scope-guard`, `source-provenance`, `cmux-doctor`) exposed from `plugins/local-skills/skills/` via `scripts/skills.sh`
...
- Codex CLI — first-class `scripts/codex.sh` setup owns `@openai/codex` install/auth, installs the official cmux Codex skill plus repo-local skills into `~/.codex/skills/`, and copies the portable `configs/codex/config.toml` template to `~/.codex/config.toml` so Codex's machine-local trust/runtime entries don't dirty the repo.
