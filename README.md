...
- Skills — agent-skills, ai-slop-cleaner, clarify, code-review, doc-coauthoring, e2e-skills, frontend-design, humanizer, im-not-ai, internal-comms, karpathy-guidelines, mcp-builder, obsidian-skills, project-session-manager (`/oh-my-claudecode:psm`), security-best-practices, skill-creator, ui-clone-skills, webapp-testing, plus repo-local skills (`dotfiles-verify`, `agent-usage-audit`, `code-intel-doctor`, `work-scope-guard`, `source-provenance`) exposed from `plugins/local-skills/skills/` via `scripts/skills.sh`
