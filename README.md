...
- Plugins — ralph-loop (iterative autonomous dev loops; `/ralph-loop:ralph-loop`), [claude-hud@claude-hud](https://github.com/jarrodwatts/claude-hud) (context/tool/agent/todo statusline; run `/claude-hud:setup` once), [skills-janitor@skills-janitor](https://github.com/khendzel/skills-janitor) (skill inventory, duplicates, token value, cleanup), [autoresearch@autoresearch](https://github.com/uditgoenka/autoresearch) (bounded metric-driven Claude loop; `/autoresearch`, `/autoresearch:debug`, `/autoresearch:fix`, etc.), [review-loop@hamel-review](https://github.com/hamelsmu/claude-review-loop) (explicit `/review-loop` Claude→Codex review loop, with `REVIEW_LOOP_CODEX_FLAGS="--sandbox workspace-write"` to avoid the plugin's dangerous default), [codex@openai-codex](https://github.com/openai/codex-plugin-cc) (delegate to / review with the local Codex CLI from inside Claude Code), security-guidance, superpowers, rust-analyzer-lsp, fakechat, vercel, session-report, claude-md-management (`/claude-md-management:revise-claude-md` + `claude-md-improver` audit skill), hookify, session-wrap, [claude-mem@thedotmack](https://github.com/thedotmack/claude-mem) (persistent memory + cross-session search), plus role-focused plugins from [wshobson/agents](https://github.com/wshobson/agents) marketplace (comprehensive-review, javascript-typescript, python-development, frontend-mobile-development, security-scanning, documentation-generation, unit-testing, tdd-workflows, git-pr-workflows, error-debugging, ui-design, accessibility-compliance, content-marketing, seo-*). Plugin slash command names follow each plugin's manifest: many use `/<plugin-name>:<command>` (e.g. `/ralph-loop:ralph-loop`, not bare `/ralph-loop`), while autoresearch exposes `/autoresearch` and `/autoresearch:<subcommand>`. Standalone skills (`/graphify`, etc.) don't take the prefix.
- Hooks — skill-eval (forced-eval prompt injection per Scott Spence pattern, ~84% activation rate), `rtk hook claude` (in-place PreToolUse hook registered via `rtk init --global`; compresses Bash output 60–90%), pretool-guard (structured PreToolUse deny for risky Bash), skill-md-edit-warn (PostToolUse reminder after editing `SKILL.md`), plus plugin hooks from autoresearch (context/privacy/loop safety), security-guidance (edit/stop security review), and review-loop (Stop hook only while an explicit `/review-loop` is active)
