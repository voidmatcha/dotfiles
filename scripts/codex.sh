...
install_codex_agents_md() {
  ...
  tmp_file=$(mktemp)
  codex_agents_compose > "$tmp_file"

  if [ -L "$dst" ]; then
    # Someone pointed this at a file of their own. The target survives, but say
    # so: every other destructive branch here warns first.
    warn "Replacing symlink $dst -> $(readlink "$dst")"
    rm "$dst"
  elif [ -e "$dst" ]; then
    if cmp -s "$tmp_file" "$dst"; then
      rm -f "$tmp_file"
